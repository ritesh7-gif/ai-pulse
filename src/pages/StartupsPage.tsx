...
import { Tool, CATEGORIES } from "../types";
...
export default function StartupsPage() {
  ...
  useEffect(() => {
    ...
    if (savedTools.length > 0) {
      setTools(mergeTools(savedTools, PERMANENT_TOOLS).sort((a, b) => b.votesCount - a.votesCount));
      setLoading(false);
    }

    fetchTools(); // Fetch fresh data from backend proxy
  ...
  const fetchTools = async (after?: string) => {
    try {
      if (after) {
        setLoadingMore(true);
      ...
      const url = after ? `/api/tools?after=${after}` : "/api/tools";
      const res = await fetch(url);

      if (!res.ok) {
        console.warn("API Route Error:", res.status);
        return;
      }

      const data = await res.json();
      if (data && Array.isArray(data.tools)) {
        const newApiTools = data.tools;

        setTools(prevTools => {
          const apiOnly = prevTools.filter(t => !t.id.startsWith('perm-'));
          const merged = after ? [...apiOnly, ...newApiTools] : newApiTools;
          ...
          if (!after) saveToolsToStorage("startups", unique);

          return mergeTools(unique, PERMANENT_TOOLS).sort((a, b) => b.votesCount - a.votesCount);
        });
  ...
  const filteredTools = useMemo(() => {
    ...
    return tools.filter((tool) => {
      ...
      if (selectedCategory === "All") {
        ...
        const topics = tool.topics.edges.map(e => e.node.name.toLowerCase());
    ...
    <div className="min-h-screen bg-bg-dark text-white/90 selection:bg-accent-purple/30 pb-20">
      <main className="max-w-7xl mx-auto px-6 pt-32">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass border-white/5 text-[9px] uppercase tracking-[0.3em] font-bold text-accent-purple mb-6 shadow-xl">
            <LayoutGrid size={12} />
          ...
          <p className="text-white/70 text-base max-w-xl mx-auto leading-relaxed font-medium">
            Explore the most innovative AI tools launching daily.
            Curated directly from top trending lists.
          </p>
        </div>
        ...
        <div className="relative max-w-2xl mx-auto mb-12">
          <div className="relative group">
            <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none text-white/80 group-focus-within:text-accent-purple transition-colors z-10">
              <Search size={20} />
            </div>
            <input
              type="text"
              placeholder="Search startups..."
              className="w-full pl-16 pr-14 py-4 bg-gradient-to-b from-white/[0.03] to-white/[0.01] backdrop-blur-md border-none rounded-2xl focus:outline-none focus:ring-2 focus:ring-accent-purple/20 hover:shadow-[0_0_30px_rgba(139,92,246,0.1)] focus:shadow-[0_0_50px_rgba(139,92,246,0.2)] transition-all duration-300 text-base placeholder:text-white/40 text-white"
              value={searchQuery}
        ...
        <div className="mb-16">
          <div className="mb-6 flex items-center justify-center gap-3 text-white/60 font-bold uppercase tracking-[0.3em] text-[9px]">
            <Filter size={12} />
            Filter by Category
          </div>
          <motion.div
        ...
        {loading ? (
          ...
          <div className="glass rounded-[40px] p-20 text-center max-w-2xl mx-auto border-white/5">
            <div className="inline-flex items-center justify-center w-24 h-24 rounded-[32px] bg-accent-cyan/10 text-accent-cyan mb-10">
              <Zap size={48} />
            </div>
            <h3 className="text-4xl font-display font-bold mb-6 tracking-tight">New Updates</h3>
            <p className="text-white/30 mb-12 leading-relaxed text-lg font-medium">
              New tools are being updated. Please check back shortly.
            </p>
            <button
              onClick={() => fetchTools()}
              className="px-12 py-5 bg-white/5 hover:bg-white/10 rounded-2xl font-bold uppercase tracking-widest text-xs transition-all"
            >
              Refresh Directory
            </button>
          </div>
          ...
          <div className="glass rounded-[40px] p-10 text-center max-w-lg mx-auto border-white/5">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-[24px] bg-accent-purple/10 text-accent-purple mb-6">
              <Search size={32} />
            </div>
            <h3 className="text-2xl font-display font-bold mb-4 tracking-tight">No Startup Tools Found</h3>
            <p className="text-white/30 mb-8 leading-relaxed text-sm font-medium">
              We couldn't find any startup tools matching your search or category.
            </p>
            <button
              onClick={() => {
                setSearchQuery("");
        ...
        {/* Load More Button */}
        {pageInfo.hasNextPage && selectedCategory === "All" && filteredTools.length > 0 && (
